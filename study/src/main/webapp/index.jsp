...
<body>
	...
	<div style="text-align: right; margin: 20px auto; max-width: 800px;">
	    <form action="searchPost.do" method="get">
	        ...
	        <input type="text" name="searchKeyword" value="<%= request.getAttribute("searchKeyword") != null ? request.getAttribute("searchKeyword") : "" %>"
	               placeholder="제목 검색" required>
	        <button type="submit">검색</button>
	        <% if(request.getAttribute("isSearch") != null) { %>
	            <a href="postList.do?category=<%= request.getAttribute("category") %>"><button type="button">목록으로</button></a>
	        <% } %>
	    </form>
	</div>
